...
if( isMainThread )
{
	...
	for( let i = 0; i < 3; ++i )
	{
		workers.push( worker = new Worker( __filename ) );
		worker.ipc = new IPC( worker );

		data.set( worker, worker_data = {});

		registerIPCListeners( worker.threadId, worker.ipc, worker_data );

		worker.postMessage( 'unknown_message' );
	}

	for( let worker of workers )
	{
		worker.ipc.emit( 'ipc_created', 'ipc_created:'+worker.threadId );
		worker.ipc.emit( 'ipc_created2', 'ipc_created2:'+worker.threadId );
		worker.ipc.emit( 'empty_event' );
	}

	for( let worker of workers )
	{
		worker.ipc.call( 'ipc_reply' ).then( r => data.get(worker).ipc_reply = ( 'ipc_replied:'+worker.threadId ));
		...
		worker.ipc.call( 'ipc_reply_topic', 'ipc_reply_topic_data' ).timeout(2000).then( r => data.get(worker).ipc_reply_topic = ( r === 'ipc_replied_topic:'+worker.threadId ));
		worker.ipc.call( 'ipc_reply_unhandled', null ).catch( e => data.get(worker).ipc_reply_unhandled = ( e === 'IPC_UNHANDLED_CALL' ));
		worker.ipc.call( 'ipc_reply_topic_skip', null ).then( r => data.get(worker).ipc_reply = ( r === 'ipc_reply_topic_skipped:'+worker.threadId ));
	}
	...
	for( let worker of workers )
	{
		...
		worker.ipc.send( 'message_topic__unhandled', 1 ).catch( e => data.get(worker).send_topic_unhandled = ( e === 'IPC_UNHANDLED_MESSAGE' ) );

		worker.ipc.send( 'message_topic_skip', 1 ).timeout(2000).then( reply =>
		{
			assert.ok( reply.data === 'ipc_message_topic_skipped:'+worker.threadId );
		});
...
{
	let ipc = new IPC( parentPort ), data = {};

	registerIPCListeners( threadId, ipc, data );

	parentPort.postMessage( 'unknown_message' );

	ipc.emit( 'ipc_created', 'ipc_created:'+threadId );
	ipc.emit( 'ipc_created2', 'ipc_created2:'+threadId );
	ipc.emit( 'empty_event' );

	ipc.call( 'ipc_reply' ).then( r => data.ipc_reply = ( 'ipc_replied:'+threadId ));
	ipc.call( 'ipc_reply_timeouted' ).timeout(1000, 'IPC_TIMEOUTED').catch( e => data.ipc_reply_timeouted = ( e === 'IPC_TIMEOUTED' ));
	ipc.call( 'ipc_reply_topic', 'ipc_reply_topic_data' ).timeout(2000).then( r => data.ipc_reply_topic = ( r === 'ipc_replied_topic:'+threadId ));
	ipc.call( 'ipc_reply_unhandled', null ).catch( e => data.ipc_reply_unhandled = ( e === 'IPC_UNHANDLED_CALL' ));
	ipc.call( 'ipc_reply_topic_skip', null ).then( r => data.ipc_reply = ( r === 'ipc_reply_topic_skipped:'+threadId ));
	...
	ipc.send( 'message_topic__unhandled', 1 ).catch( e => data.send_topic_unhandled = ( e === 'IPC_UNHANDLED_MESSAGE' ) );

	ipc.send( 'message_topic_skip', 1 ).timeout(2000).then( reply =>
	{
		assert.ok( reply.data === 'ipc_message_topic_skipped:'+threadId );
	});
